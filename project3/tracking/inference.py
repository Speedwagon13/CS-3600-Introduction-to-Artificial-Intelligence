    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for num in range (0, self.numParticles):
                jailParticles.append(self.getJailPosition())
            self.particles = jailParticles
        else:
            allPossible = util.Counter()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        configurations = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for particle in (self.particles):
            ...
            particleList[particle] += prob

        particleList.normalize()

        if (particleList.totalCount() == 0):
            self.initializeParticles()
            for particle in self.particles:
                for i in range (self.numGhosts):
            ...
            for num in range(self.numParticles):
                newParticles.append(util.sample(particleList))
