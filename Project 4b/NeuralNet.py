...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        """YOUR CODE"""
        withOne = [1.0]
        for element in inActs:
            withOne.append(element)

        weightedSum = self.getWeightedSum(withOne)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        index = 0
        for i in range(len(withOne)):
            oldWeight = self.weights[index]
            newWeight = oldWeight + alpha * delta * withOne[index]
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            ...
            for perceptron in layer:
                perceptrons.append(perceptron.sigmoidActivation(previous))
            inAndOut.append(perceptrons)
            previous = perceptrons
        return inAndOut
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            for outputNum in xrange(len(example[1])):
                ...
                error = example[1][outputNum] - lastLayerOutput[outputNum]   # maybe other way around
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    for nextNeuronNum in xrange(len(nextLayer)):
                        delta = deltas[0][nextNeuronNum]
                        weight = nextLayer[nextNeuronNum].weights[neuronNum + 1]
                        nextError += delta * weight
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    while ((weightMod > weightChangeThreshold) and (iteration <= maxItr)):
        ...
        iteration += 1
        if iteration%10==0:
            print "! iteration: " + str(iteration) + "; weight change: " + str(weightMod)
        else :
            print '.',
    ...
    for example in examplesTest:
        ourOut = nnet.feedForward(example[0])[-1]
        correct = True
        for index in range(len(ourOut)):
            if (round(ourOut[index]) != example[1][index]):
                correct = False
        if correct:
            testCorrect += 1
        else:
